...
export default function ContractForm({
  ...
  useEffect(() => {
    ...
    if (editBooking) {
      ...
      setTimePending(editBooking.timePending || false);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const bookingData: Booking = {
      ...
      address,
      eventDate,
      eventTime: timePending ? "" : eventTime,
      phone,
      packageId,
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    id="input-event-time"
                    type="time"
                    value={timePending ? "" : eventTime}
                    disabled={timePending}
                    required={!timePending}
                    onChange={(e) => setEventTime(e.target.value)}
                    className="w-full pl-10 pr-4 py-2 bg-white/60 border border-white/50 rounded-xl focus:outline-none focus:ring-2 focus:ring-brand-orange/50 focus:border-brand-orange text-sm transition-all font-mono shadow-inner disabled:bg-slate-100 disabled:text-slate-400"
                  />
                </div>
                ...
                <div className="mt-2 flex items-center gap-2">
                  <input
                    ...
                    checked={timePending}
                    onChange={(e) => {
                      setTimePending(e.target.checked);
                  ...
                  <label
                    htmlFor="time-pending"
                    className="text-sm text-slate-600 font-medium"
                  >
                    Horario pendiente por definir
                  </label>
                </div>
                {!timePending && eventTime && (
              ...
              {PACKAGES.map((pkg) => {
                ...
                return (
                  <div
                    ...
                    className={`p-3 rounded-2xl cursor-pointer border-2 transition-all flex justify-between items-center ${isSelected
            ...
            <div className="space-y-3 max-h-72 overflow-y-auto pr-1">
              {EXTRA_SERVICES.map((service) => {
                ...
                return (
                  <div
                    key={service.id}
                    className={`p-3 rounded-2xl border transition-all space-y-2 ${qty > 0 ? "border-brand-blue/40 bg-blue-50/20 shadow-sm" : "border-slate-100 bg-slate-50/50"
                      }`}
                  >
